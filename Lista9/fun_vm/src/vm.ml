...
let flatten (c: cmd list) : T.cmd list =
  ...
  let next_label_suffix = fun () -> counter := (!counter) + 1; "L" ^ string_of_int(!counter) in
  let rec inner (c: cmd list) : T.cmd list =
    ...
    | CondJmp(c1, c2) :: c' -> 
      let label1, label2 = next_label_suffix (), next_label_suffix () in
      [T.CondJmp label1] @ inner c2 @ [T.Jmp label2; T.Lbl label1] @ inner c1 @ [T.Lbl label2]
(* 
      [T.Lbl label1] @ (inner c1) @ [T.Return] @ [T.Lbl label2] @ (inner c2) @ [T.Return] 
      @ [T.CondJmp label1] @ [T.Jmp label2] @ (inner c')
*)
    | Grab :: c' -> [T.Grab] @ (inner c')
    | Access n :: c' -> [T.Access n] @ (inner c')
    | EndLet :: c' -> [T.EndLet] @ (inner c')
    | PushClo c :: c' -> 
        let label1, label2 = next_label_suffix (), next_label_suffix() 
        in [T.Jmp label2, T.Lbl label] @ (inner c) @ [T.Return] @ [T.Lbl label2; T.PushClo label] @ (inner c')
